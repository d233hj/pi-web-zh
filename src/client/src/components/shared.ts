...
export const appStyles = css`
  ...
  .shell { display: grid; grid-template-columns: var(--pi-nav-width, 260px) 4px minmax(280px, 1fr) 4px var(--pi-panel-width, minmax(280px, 36vw)); height: 100%; min-height: 0; transition: none; }
  .grid-splitter { position: relative; z-index: 10; width: 4px; margin: 0 -2px; background: var(--pi-border-muted); cursor: col-resize; user-select: none; -webkit-user-select: none; transition: background .15s; }
  .grid-splitter::after { content: ""; position: absolute; inset: 0 -4px; }
  .grid-splitter:hover, .grid-splitter.active { background: var(--pi-accent); }
  .shell.resizing { cursor: col-resize; user-select: none; -webkit-user-select: none; }
  .shell.resizing > * { pointer-events: none; }
  .shell.resizing .grid-splitter { pointer-events: auto; }
