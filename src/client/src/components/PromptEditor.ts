    ...
    return html`
      <footer class=${shellMode ? "shell-mode" : ""}>
        ...
        <div class="actions">
          ...
          <button ?disabled=${this.disabled} title=${queuesInput ? t("composer.queueTitle") : t("composer.send")} @click=${() => { this.send("followUp"); }}>${queuesInput ? t("composer.queue") : t("composer.send")}</button>
          ${this.canSteer && !this.isCompacting ? html`<button ?disabled=${this.disabled} title=${t("composer.steerTitle")} @click=${() => { this.send("steer"); }}>${t("composer.steer")}</button>` : null}
          <button ?disabled=${this.disabled || !this.canStop} title=${this.canStop ? t("composer.stopTitle") : t("composer.nothingRunning")} @click=${() => this.onStop?.()}>${t("composer.stop")}</button>
        </div>
      </footer>
    `;
  }
  ...
  private renderCompactStatus() {
    ...
    return html`
      <div class="compact-status" aria-label="Session status">
        ...
        <button class="select-thinking" title=${t("composer.selectThinking")} @click=${() => this.onSelectThinking?.()}>think ${status.thinkingLevel ?? "off"}</button>
      </div>
    `;
  }
