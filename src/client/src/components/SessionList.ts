...
export class SessionList extends LitElement {
  ...
  private renderHeading(sessionCount: number) {
    ...
    const selectedSummary = this.selected === undefined ? t("sessionList.noSessionSelected") : sessionLabel(this.selected);
    ...
    return html`
      ...
      >
        ...
        <div class="action-menu">
          <button class="action-menu-toggle" title=${t("sessionList.sessionActions")} @click=${(event: MouseEvent) => { event.stopPropagation(); this.toggleMenu(session.id, event.currentTarget); }}>⋯</button>
          ${this.openMenuSessionId === session.id ? html`
            <div class="action-menu-panel" style=${this.menuStyle}>
              ${session.parentSessionPath !== undefined ? html`<button title=${t("sessionList.detachFromParent")} @click=${() => { this.openMenuSessionId = undefined; this.onDetachParent?.(session); }}>${t("sessionList.detachFromParent")}</button>` : null}
              ${isCachedNewSessionInfo(session)
                ? html`<button title=${t("sessionList.delete")} @click=${() => { this.openMenuSessionId = undefined; this.onDelete?.(session); }}>${t("sessionList.delete")}</button>`
                : session.archived === true
                  ? html`<button title=${t("sessionList.restore")} @click=${() => { this.openMenuSessionId = undefined; this.onRestore?.(session); }}>${t("sessionList.restore")}</button>`
                  : html`
                    <button title=${t("sessionList.archive")} @click=${() => { this.openMenuSessionId = undefined; this.onArchive?.(session); }}>${t("sessionList.archive")}</button>
                    ${descendantCount > 0 ? html`<button title=${t("sessionList.archiveWithDescendants")} @click=${() => { this.openMenuSessionId = undefined; this.confirmArchiveWithDescendants(session, descendantCount); }}>${t("sessionList.archiveWithDescendants")} (${descendantCount})</button>` : null}
                  `}
            </div>
          ` : null}
        </div>
  ...
  private renderStatus(session: SessionInfo) {
    ...
    return renderActivityIndicator(isSessionActive(this.statuses[session.id], this.activities[session.id]) ? "session" : undefined, t("sessionList.sessionActive")) ?? "";
  }
