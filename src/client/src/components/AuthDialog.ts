...
export class AuthDialog extends LitElement {
      ...
      <div class="backdrop" @mousedown=${() => { this.cancel(); }}>
        <section @mousedown=${(event: MouseEvent) => { event.stopPropagation(); }} @keydown=${(event: KeyboardEvent) => { this.handleKeyDown(event); }}>
          <header>
            ...
            <button title=${t("dialog.close")} @click=${() => { this.cancel(); }}>×</button>
          </header>
  ...
  private dialogTitle(state: AuthDialogState): string {
    switch (state.step) {
      case "method": return t("dialog.configureAuth");
      case "providers": return state.authType === undefined ? t("dialog.selectAuth") : state.authType === "oauth" ? t("dialog.selectSubscription") : t("dialog.selectApiKey");
      case "apiKey": return `API key for ${state.provider.name}`;
      case "oauth": return `Login to ${state.flow.providerName}`;
      case "logout": return t("auth.logoutTitle");
    }
  }
    ...
    switch (state.step) {
      ...
      case "apiKey": return html`
        <div class="form">
          ...
          <input type="password" autocomplete="off" placeholder=${t("auth.apiKeyPlaceholder")} .value=${state.value} @input=${(event: Event) => { if (event.target instanceof HTMLInputElement) this.onApiKeyInput?.(event.target.value); }}>
          ${state.error !== undefined && state.error !== "" ? html`<div class="error-text">${state.error}</div>` : null}
          <div class="actions"><button @click=${() => { this.cancel(); }}>${t("dialog.cancel")}</button><button class="primary" ?disabled=${state.saving === true} @click=${() => { this.onSaveApiKey?.(); }}>${state.saving === true ? t("dialog.saving") : t("dialog.saveApiKey")}</button></div>
        </div>
      `;
...
function authTypeLabel(authType: "oauth" | "api_key"): string {
  return authType === "oauth" ? t("dialog.subscription") : t("dialog.apiKey");
}
