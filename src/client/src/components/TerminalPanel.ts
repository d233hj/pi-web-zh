    ...
    if (isCommandRunPending(run)) {
      ...
      return html`
        <section class="command-run-notice running">
          ...
          <button class="danger" ?disabled=${cancelling} @click=${() => { void this.cancelCommandRun(run); }}>${cancelling ? t("terminal.cancelSent") : t("terminal.cancelCommand")}</button>
        </section>
      `;
    }
    if (terminal?.exited === true) {
      ...
      return html`
        <section class=${`command-run-notice ${run.status}`}>
          ...
          <button ?disabled=${continuing} @click=${() => { void this.continueTerminal(terminal.id); }}>${continuing ? t("terminal.startingShell") : t("terminal.continueInShell")}</button>
        </section>
      `;
    }
    return null;
