...
export class WorkspaceList extends LitElement {
  ...
  private renderHeading() {
    if (!this.collapsible) return t("workspaceList.workspaces");
    const selectedSummary = this.selected === undefined ? t("workspaceList.noWorkspaceSelected") : `${this.selected.label}${this.selected.isMain ? t("general.main") : ""} · ${this.selected.path}`;
  ...
  private renderActivity(workspace: Workspace): TemplateResult | undefined {
    ...
    return renderActivityIndicator(kind, kind === "terminal" ? t("workspaceList.terminalActive") : t("workspaceList.active"));
  }
    ...
    return html`
      <div class="action-menu">
        <button
          ...
          title=${t("workspaceList.actionsAndDetails")}
  ...
  private renderWorkspaceActions(workspace: Workspace): TemplateResult | undefined {
    ...
    return html`
      <div class="workspace-menu-actions">
        <button class="danger" title=${deleting ? t("workspaceList.deletionInProgress") : t("workspaceList.deleteWorkspace")} ?disabled=${deleting} @click=${() => { this.delete(workspace); }}>${deleting ? t("workspaceList.deleting") : t("workspaceList.deleteWorkspace")}</button>
      </div>
    `;
  }
    ...
    return html`
      <dl class="workspace-menu-details">
        <div class="workspace-detail-row">
          <dt>${workspace.branch === undefined ? t("general.workspace") : t("general.branch")}</dt>
          <dd>${label}</dd>
        </div>
        <div class="workspace-detail-row">
          <dt>Path</dt>
