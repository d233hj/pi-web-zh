...
export class PiWebApp extends LitElement {
  ...
  @query(".shell") private shellEl?: HTMLElement;
    ...
    return html`
      <header>
        ...
        <div class="header-actions">
          ...
          <button title=${t("actions.showActions")} aria-label=${t("actions.showActions")} @click=${() => { this.setState({ actionPaletteOpen: true }); }}>${t("actions.label")}</button>
        </div>
      </header>
      <project-list
  ...
  private workspacePanelEmptyState(): WorkspacePanelEmptyState {
    ...
    if (project === undefined) {
      return this.state.projects.length === 0
        ...
        : {
            title: t("project.selectTitle"),
            body: t("project.selectBody"),
          };
    }
    if (this.state.isLoadingWorkspaces) {
      return {
        title: t("workspace.loading"),
        body: `${t("workspace.loading")} ${project.name}`,
      };
    }
    if (this.state.workspaces.length === 0) {
      return {
        title: t("workspace.noWorkspacesFound"),
        body: `${project.name}${t("workspace.noWorkspacesFound")}`,
      };
    }
    return {
      title: t("workspace.selectWorkspace"),
      body: `${t("workspace.selectWorkspace")} — ${project.name}`,
    };
  }

  private sessionEmptyMessage(): string {
    if (this.state.isLoadingProjects) return t("project.loading");
    if (this.state.selectedWorkspace !== undefined) return t("session.selectOrStart");
    ...
    return t("session.selectProjectAndWorkspace");
  }
  ...
  private async deleteWorkspace(workspace = this.state.selectedWorkspace): Promise<void> {
    ...
    if (!canDeleteWorkspace(workspace)) {
      this.setState({ error: t("error.onlySecondaryGitWorktrees") });
      return;
    }
    ...
    try {
      ...
      if (mainWorkspace === undefined) {
        this.setState({ error: t("error.projectMainWorkspaceNotFound") });
        return;
      }
  ...
  private async handleCompletedWorkspaceDeletionRun(run: TerminalCommandRun): Promise<void> {
    ...
    if (run.status === "failed") {
      this.setState({ error: t("error.workspaceDeletionFailed") });
      this.updateWorkspaceDeletionPolling();
    }
  }
  ...
  private autoThemeDescription(resolution: ThemePreferenceResolution): string {
    if (!this.themePreference.auto) return t("theme.autoOffDesc");
    if (resolution.selectedTheme === undefined) return t("theme.autoOffDesc");
    if (resolution.selectedThemePair === undefined) return t("theme.autoOnNoPair");
    const mode = this.systemPrefersLight() ? "light" : "dark";
    return `On · ${resolution.selectedThemePair.name} follows the system ${mode} preference.`;
  }
  ...
  private async openThinkingDialog() {
    ...
    this.setState({
      thinkingDialog: {
        title: t("dialog.selectThinkingLevel"),
  ...
  private renderAppRefresh() {
    const label = this.isRefreshingApp ? t("refresh.refreshing") : t("refresh.normal");
    return html`
  ...
  private renderRefreshMenu() {
    ...
    return html`
      <div class="app-refresh-menu" role="menu" style=${this.refreshMenuStyle} @click=${(event: MouseEvent) => { event.stopPropagation(); }}>
        <button role="menuitem" @click=${() => { void this.refreshAppData(); }}>${t("refresh.normal")}</button>
        <button role="menuitem" @click=${() => { this.refreshMenuOpen = false; this.suppressNextRefreshClick = false; this.hardReloadApp(); }}>${t("refresh.fullReload")}</button>
      </div>
    `;
  }
  ...
  private getStoredWidths(): { nav?: number; panel?: number } {
    try {
      const raw = localStorage.getItem("pi-web-panel-widths");
      if (raw) return JSON.parse(raw) as { nav?: number; panel?: number };
    } catch { /* ignore */ }
    return {};
  }

  private saveWidths(nav: number, panel: number): void {
    try { localStorage.setItem("pi-web-panel-widths", JSON.stringify({ nav, panel })); } catch { /* ignore */ }
  }

  private applyStoredWidths(): void {
    const shell = this.shellEl;
    if (!shell) return;
    const stored = this.getStoredWidths();
    if (stored.nav) shell.style.setProperty("--pi-nav-width", `${stored.nav}px`);
    if (stored.panel) shell.style.setProperty("--pi-panel-width", `${stored.panel}px`);
  }

  private onSplitterDown(e: MouseEvent, which: "nav" | "panel"): void {
    e.preventDefault();
    const shell = this.shellEl;
    if (!shell) return;
    this.draggingSplitter = which;
    this.dragStartX = e.clientX;
    const prop = which === "nav" ? "--pi-nav-width" : "--pi-panel-width";
    const current = shell.style.getPropertyValue(prop);
    this.dragStartWidth = current ? parseInt(current, 10) : (which === "nav" ? 260 : 360);
    shell.classList.add("resizing");
    document.addEventListener("mousemove", this.onSplitterMove);
    document.addEventListener("mouseup", this.onSplitterUp);
  }

  private handleSplitterMove(e: MouseEvent): void {
    if (!this.draggingSplitter || !this.shellEl) return;
    const delta = e.clientX - this.dragStartX;
    const which = this.draggingSplitter;
    // For nav splitter: drag right = wider nav; for panel: drag left = wider panel
    const sign = which === "nav" ? 1 : -1;
    const newWidth = Math.max(which === "nav" ? 180 : 200, Math.min(which === "nav" ? 500 : 800, this.dragStartWidth + sign * delta));
    const prop = which === "nav" ? "--pi-nav-width" : "--pi-panel-width";
    this.shellEl.style.setProperty(prop, `${newWidth}px`);
  }

  private handleSplitterUp(): void {
    document.removeEventListener("mousemove", this.onSplitterMove);
    document.removeEventListener("mouseup", this.onSplitterUp);
    if (this.shellEl) {
      this.shellEl.classList.remove("resizing");
      const navRaw = this.shellEl.style.getPropertyValue("--pi-nav-width");
      const panelRaw = this.shellEl.style.getPropertyValue("--pi-panel-width");
      this.saveWidths(
        navRaw ? parseInt(navRaw, 10) : 260,
        panelRaw ? parseInt(panelRaw, 10) : 360,
      );
    }
  ...
  override render() {
    ...
    return html`
      <div class=${`shell ${state.mainView === "navigation" ? "navigation-view" : state.mainView === "chat" ? "chat-view" : "workspace-view"}`}>
        ...
        <div class="grid-splitter" @mousedown=${(e: MouseEvent) => this.onSplitterDown(e, "nav")}></div>
        <main class=${state.mainView === "chat" ? "chat-view" : state.mainView === "navigation" ? "navigation-view" : "workspace-view"}>
          ...
          <div class=${this.mobileTabsFrameClass()}>
            <div class="mobile-tabs" @scroll=${this.onMobileTabsScroll}>
              <button class=${state.mainView === "navigation" ? "mobile-navigation-tab selected" : "mobile-navigation-tab"} @click=${() => { this.selectMainView("navigation"); }}>${t("nav.sessions")}</button>
              <button class=${state.mainView === "chat" ? "selected" : ""} @click=${() => { this.selectMainView("chat"); }}>${t("nav.chat")}</button>
...
function projectContextLabel(project: Project | undefined): string {
  return project?.name ?? t("project.noProject");
}

function projectContextTitle(project: Project | undefined): string {
  return project === undefined ? t("project.noProjectSelected") : `${project.name} — ${project.path}`;
}

function workspaceContextLabel(workspace: Workspace | undefined): string {
  return workspace === undefined ? t("workspace.noWorkspace") : `${workspace.label}${workspace.isMain ? t("workspace.main") : ""} · ${workspace.path}`;
}

function workspaceContextTitle(workspace: Workspace | undefined): string {
  return workspace === undefined ? t("workspace.noWorkspaceSelected") : `${workspace.label}${workspace.isMain ? t("workspace.main") : ""} — ${workspace.path}`;
}

function sessionContextLabel(session: SessionInfo | undefined): string {
  const name = session?.name?.trim();
  const firstMessage = session?.firstMessage.trim();
  return name !== undefined && name !== "" ? name : firstMessage !== undefined && firstMessage !== "" ? firstMessage : session?.id.slice(0, 8) ?? t("session.noSession");
}

function sessionContextTitle(session: SessionInfo | undefined): string {
  return session === undefined ? t("session.noSessionSelected") : session.path;
}
...
function thinkingDescription(level: ThinkingLevel): string {
  switch (level) {
    case "off": return t("thinking.off");
    case "minimal": return t("thinking.minimal");
    case "low": return t("thinking.low");
    case "medium": return t("thinking.medium");
    case "high": return t("thinking.high");
    case "xhigh": return t("thinking.xhigh");
  }
}
