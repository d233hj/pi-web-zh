  ...
  override render() {
    ...
    return html`
      <section class=${`tool-card ${execution.status}`}>
        ...
        ${visibleDiff === undefined ? this.renderTextBody(bodyText, execution.status === "error") : this.renderDiffBody(visibleDiff, actualDiff === undefined ? t("diff.previewDiff") : t("diff.appliedDiff"))}
    ...
    return html`
      <details class="diff-details" ?open=${this.diffOpen} @toggle=${(event: Event) => { this.onDiffToggle(event); }}>
        ...
        <div class="diff-toolbar">
          <span>${truncated ? t("diff.showingLines", String(visibleLines.length), String(lines.length)) : t("diff.fullDiff")}</span>
          <button type="button" @click=${() => { void this.copyDiff(diff); }}>${this.copied ? t("diff.copied") : t("diff.copyDiff")}</button>
        </div>
