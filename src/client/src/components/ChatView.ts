...
function getPartialStreamNoticeBodies(): readonly string[] {
  return [
    t("chat.lateArrival1"),
    t("chat.lateArrival2"),
    t("chat.lateArrival3"),
...
function randomPartialStreamNoticeBody(): string {
  const bodies = getPartialStreamNoticeBodies();
...
export class ChatView extends LitElement {
      ...
      <aside class="queued-messages" aria-live="polite">
        ...
        ${queued.map((message, index) => html`
          <div class="queued-message">
            <span class="queued-kind">${t(message.kind === "steer" ? "chat.steer" : "chat.followUp")} ${String(index + 1)}</span>
  ...
  private renderMessageActions(message: ChatLine, key: string) {
    ...
    return html`
      <div class="msg-actions" aria-label="Message actions">
        <button type="button" class="msg-action" title=${copied ? t("chat.copied") : t("chat.copy")} aria-label=${`${copied ? t("chat.copied") : t("chat.copy")} ${message.role} message`} @click=${(event: MouseEvent) => { void this.copyMessage(message, key, event); }}>
  ...
  private messageMetaLabel(message: ChatLine): { short: string; full: string } {
    ...
    if (timestamp === undefined && model === undefined) {
      const empty = { short: t("chat.noInfo"), full: t("chat.noMetadata") };
