...
function renderDiffViewer(context: WorkspacePanelContext): TemplateResult {
  ...
  if (unstaged === undefined || staged === undefined) return html`<p class="muted">${t("panels.loadingDiff")}</p>`;
  const diffs = [staged, unstaged].filter((diff) => diff.diff !== "");
  if (diffs.length === 0) return html`<p class="muted">${t("panels.noDiff")}</p>`;
  return html`
...
function renderDiffSection(diff: GitDiffResponse): TemplateResult {
  ...
  return html`
    <section class="diff-section">
      <div class="viewer-header"><strong>${diff.path ?? t("panels.diff")}</strong><small>${diff.staged ? t("panels.staged") : t("panels.unstaged")}${diff.truncated ? t("panels.truncatedSuffix") : ""}</small></div>
